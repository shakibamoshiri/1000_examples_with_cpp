...
template<class MRI>         // declaration
std::reverse_iterator<MRI>  // return type
mri (MRI what){return std::reverse_iterator<MRI>(what);} // make reverse iterator function
...
int main(){
    ...
    const float* evf=&(*std::end(vfloat)); // pointer to end of vfloat

    while(evf!=&(*std::begin(vfloat)))std::cout<<*--evf<<' '; // print form end-1 to begin
