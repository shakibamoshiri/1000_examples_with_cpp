...
int main(){
    ...
    // like vector but for array C-type
   int vector[]={12,24,36,48,60};
   // but array C-type has no begin() member and end
   // so you should ues std::begin() and std::end()

   std::find(std::begin(vector),std::end(vector),36)!=std::end(vector) // using this syntax: returned-boolean-expression ? does-for-true : does-for-false
             ? std::cout<<"okay found\n"
