...
int main(){
    ...
    std::remove_copy_if(str.begin(),str.end(),std::back_inserter(rem),[](char c){ return c==' ';});
    // Removes all space in the string, if that predicate returns true.
    // char c, no need to call by reference, because it does not modify the original string
