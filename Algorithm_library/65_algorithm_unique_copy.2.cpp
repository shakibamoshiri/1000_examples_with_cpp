...
//
/// unique_copy
// Copies the elements form the range [first, last), to another range beginning
//  at d_first n such way that there are no consecutive equal elements.



int main(){
    std::string str("   A   string with          many              spaces.    ");

    std::string unique_str;
    std::unique_copy(str.begin(),                       // beginning
                     str.end(),                         // ending
                     std::back_inserter(unique_str),    // insert to an empty container
                     [](char c1,char c2){ return std::isspace(c1) && std::isspace(c2) && c1==c2;}); // using predicate function
    std::cout<<"unique_str:\t\t"<<unique_str<<std::endl;
