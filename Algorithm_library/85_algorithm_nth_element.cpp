...
int main(){
    ...
    std::nth_element(std::begin(v),
                     std::begin(v)+1,       // point to 2th element
                     std::end(v),
                     std::greater<int>()); // using comparison function
    std::cout<<"The second largest element is "<<*(std::begin(v)+1)<<'\n';

    std::cout<<"\nv:\t";
    for(const int i:v){ if (i==*(std::begin(v)+1) ) std::cout<<'*'<<i<<' '; else std::cout<<i<<' ';}
}
