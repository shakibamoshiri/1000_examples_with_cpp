...
/// std::match_results prefix and suffix
...
int main(  ){
    ...
    std::basic_regex< char > regex( "\\d+" );   // only digits
    ...
    if( m_result.ready() ){
        std::cout << "prefix ( $' ) is: " << m_result.prefix() << '\n';
        std::cout << "match ( $& )  is: " << m_result.str() << '\n';
        std::cout << "suffix ( $` ) is: " << m_result.suffix() << '\n';
    }
...
prefix ( $' ) is: Do you have
match ( $& )  is: 10
suffix ( $` ) is:  millions dollars?
