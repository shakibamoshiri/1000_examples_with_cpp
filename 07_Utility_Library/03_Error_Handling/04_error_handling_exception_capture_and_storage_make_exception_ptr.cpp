...
//  Creates an std::exception_ptr that holds a reference to a copy
//  of 'e' (parameter). This is down as if executing the following code:
/*
    try { throw e; }
...
int main(){
    ...
    catch (...) { eptr = std::make_exception_ptr(nullptr); } // the site has no example of std::make_exception_ptr, so I am sure about this example
