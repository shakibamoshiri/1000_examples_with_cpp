...
int main (){
    ...
    std::function<  void (int) > fj_add_display2= std::bind ( &St0::print_add,one,_1);
    fj_add_display2(20);

    // store a call to member function and object ptr
    std::function< void (int) > f_add_display3 = std::bind( &St0::print_add, &one, _1);
    f_add_display3(2222);

    // store a call to a function object
    std::function< void (int) > f_display_ojb = PrintNumber();
    f_display_ojb(33);
}
