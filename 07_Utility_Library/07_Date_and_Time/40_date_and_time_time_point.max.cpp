...
int main (){
    ...
    int64_t max_size = std::chrono::time_point< std::chrono::_V2::system_clock >::duration::max().count();
    std::cout << max_size << std::endl; // #1

    max_size = std::chrono::_V2::system_clock::duration::max().count();
    std::cout << max_size << std::endl; // #2

    std::cout << std::chrono::system_clock::duration::max().count() << std::endl;   // #3

    std::cout << std::chrono::duration< int64_t >::max().count() << std::endl;      // #4
    std::cout << std::chrono::duration< int32_t >::max().count() << std::endl;
    std::cout << std::chrono::duration< int16_t >::max().count() << std::endl;
}
