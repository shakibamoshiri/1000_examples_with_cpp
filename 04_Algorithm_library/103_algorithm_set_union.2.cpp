...
int main(){
    ...
    std::set_union(
                    v1.begin(),v1.end(),
                    v2.begin(),v2.end(),
                    std::copy(v2.begin()+v2.size(), // because of adding v2 twice, v2.size() remove that, test once with v2.begin(),v2.end() so can see what will happen
                               v2.end(),
                              std::ostream_iterator<int>(std::cout<<"result:\t"," ")) );
