...
int main(){
    ...
    // NOTE:
    // Although std::accumulate performed left ford by default, right fold
    // may be achieved by using reverse iterator, e.g. std::accumulate( con.rbegin(), con. rend(), init, binaryPredicate )
    std::string str = std::accumulate(std::rbegin(arr),std::rend(arr),
