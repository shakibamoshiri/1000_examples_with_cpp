...
short howManyWordAreHere(Container& con,T tmp){
    short count=0;
    short bank=0;

    for(T s : con){     // T is a std::string type

        // if (s != tmp)   count=0; only uses for consecutive search
        if (s == tmp)   ++count;
...
int main(){
    ...
    std::cout<<howManyWordAreHere(vs,std::string("A"))<<std::endl;
