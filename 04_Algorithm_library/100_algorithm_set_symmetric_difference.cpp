...
int main(){
    ...
    // new vector for storing the result
    std::vector<int> v_symmetric_difference;

    // using
    std::set_symmetric_difference(
                                  v1.begin(),v1.end(),
                                  v2.begin(),v2.end(),
                                  std::back_inserter(v_symmetric_difference));

    // custom for_range_base
    const int* bv_s = &(*std::begin(v_symmetric_difference));
    const int* ev_s = &(*std::end(v_symmetric_difference));
    while ( bv_s != ev_s) std::cout<<*bv_s++<<' ';
