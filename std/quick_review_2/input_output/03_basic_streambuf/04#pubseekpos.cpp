...
int main(){
    ...
    long size = sb->pubseekoff( 0, std::ios_base::end );

    if( size > 20 ){
        char buffer[ 11 ];
        sb->pubseekpos( 10 );       // change the position to 10th character
        sb->sgetn( buffer, 10 );    // read the 10 characters
...
----
first line of file:
If the get pointer (gptr) has a value and this is less than the end pointer (egptr)
