...
// user-defined operator ""_min
constexpr std::chrono::minutes operator ""_min(unsigned long long minutes){
    return std::chrono::minutes( minutes );
}

int main (){

    std::chrono::duration<unsigned int, std::ratio<60, 1>> lesson = 33_min;

    std::cout << "33 minutes equals to "
              << std::chrono::seconds(lesson).count() << " seconds\n";

    std::cout << "1  minute is: "
              << std::chrono::seconds(1_min).count() << " seconds\n"
              << "and it is: "
              << std::chrono::milliseconds(1_min).count() << " milliseconds\n"
              << "and it is: "
              << std::chrono::microseconds(1_min).count() << " microseconds"
              << std::endl;
...
33 minutes equals to 1980 seconds
1  minutes is: 60 seconds
and it is: 60000 milliseconds
and it is: 60000000 microseconds
