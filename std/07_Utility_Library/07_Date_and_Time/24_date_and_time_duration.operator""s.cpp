...
int main (){
    ...
    std::chrono::duration< unsigned int, std::ratio<1, 1> > half_a_min = 30s;

    std::cout << "half a minute equals to: "
              << half_a_min.count() << std::endl;

    std::chrono::minutes m = std::chrono::duration_cast< std::chrono::minutes >(half_a_min);

    std::cout << m.count() << std::endl; // it outputs 0, why? I do not know now :(
//error: conversion from ‘std::chrono::duration<unsigned int, std::ratio<1ll> >’
//to non-scalar type ‘std::chrono::minutes {aka std::chrono::duration<long long int, std::ratio<60ll> >}’ requested|
