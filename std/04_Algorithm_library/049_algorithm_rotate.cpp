...
int main(){
    ...
    std::cout<<"Before sort:\t";        for(const int i : vec) std::cout<<i<<' ';

    // insertion sort:
    // i is equivalent to std::vector<int>::iterator i
    for (auto i = vec.begin(); i != vec.end(); ++i) std::rotate(std::upper_bound(vec.begin(), i, *i), i, i+1);

    std::cout<<"\nAfter sort:\t";       for(const int i : vec) std::cout<<i<<' ';
}
